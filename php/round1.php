...
if($_SESSION["li"] == "good2go"){
	if(isset($_POST["uid"])){
		...
		if($g2g == 1){
			$sql = "INSERT INTO eggdrop.drops (uid, round, dweight, eweight, zone, cracked) VALUES (".$uid.",".$round.",".$dweight.",".$eweight.",".$zone.",".$cracked.")";
			...
			<div class="header clearfix">
				<nav>
					<ul class="nav nav-pills pull-right">
						...
						<li role="presentation" class="active"><a href="#">Round 1</a></li>
				...
				<form action="round1.php" method="post">
					<div class="form-group">
						...
						<select type="text" class="form-control" id="uid" name = "uid">
							<?php
								$sql = "SELECT z.* FROM (select a.*, b.`round` from `eggdrop`.`people` as a left join `eggdrop`.`drops` as b on a.`uid` = b.`uid`) as z WHERE year = ".date('Y')." and `round` is null order by case `tname` when '' then `fname` else `tname` end";
